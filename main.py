...
reload = startup_config.get("reload", False)
...
async def lifespan(app: FastAPI):
    try:
        print(f"{title} v{version} started successfully on http://{host}:{port}", flush=True)
...
if __name__ == "__main__":
    uvicorn.run(app, host=host, port=port, reload=reload, log_level="error")